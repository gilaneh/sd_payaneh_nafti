...
class ReportSdPayanehNaftiContractDailyReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if docids:
            contract_records = self.env['sd_payaneh_nafti.contract_registration'].browse(docids)
            # contract_no = input_record.contract_no
            calendar = context.get('lang')
        else:
            registration_no = form_data.get('registration_no')[1]
            contract_records = self.env['sd_payaneh_nafti.contract_registration'].search([('registration_no', '=', registration_no)])
            calendar = form_data.get('calendar')
            docids = [contract_records.id]
        # REPORT DATE
